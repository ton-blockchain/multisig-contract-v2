...
export const promptInt = async (prompt:string, provider:UIProvider, min?: number, max?: number) => {
    ...
    do {
        ...
        if(isNaN(resAmount)) {
            provider.write("Failed to convert " + inputAmount + " to integer number");
        } else if (!Number.isInteger(resAmount)) {
            provider.write(`The number must be integer`);
        } else if (min !== undefined && (resAmount < min)) {
            provider.write(`The number must be at least ${min}`);
        } else if (max !== undefined && (resAmount > max)) {
            provider.write(`The number must be no more than ${max}`);
        } else {
            return resAmount;
        }
    } while(true);
...
export const promptBigInt = async (prompt:string, provider:UIProvider, min?: number, max?: number) => {
    let resAmount:bigint;
    do {
        let inputAmount = await provider.input(prompt);
        resAmount = BigInt(inputAmount);
        if (resAmount < 0) {
            provider.write("Please enter non-negative number");
        } else {
            return resAmount;
        }
    } while(true);
...
export const codeFromLibrary = (codeRaw: Cell) => {
